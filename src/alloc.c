...
// !Memory
    ...
    if (HEAD == NULL) {
        ...
        while (curr_node != NULL) {
            if (size + sizeof(header) <= curr_node->size) {// Enough space in this free block
...
void tufree(void *ptr) {
    ...
    header* free_header = (header*)ptr-sizeof(header); //could be wrong
        printf("[tufree] Freeing block at %p | header at %p | magic = 0x%x | size = %zu\n",
        ptr, free_header, free_header->magic, free_header->size);
    
    if (free_header->magic == 0x01602848){
